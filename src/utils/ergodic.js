...
const mapTreeData = ({ treeData, childrenCallback = () => {}, itemCallback = () => {}, nextKey = 'children' }) => {
...
const getPrefixTreeData = (data, filter = () => true, nextKey = 'children', treeData = []) => {
  for (let [index, item] of [...data].entries()) {
    if (filter(item)) {
      treeData.push({ ...item })
      if (item[nextKey].length) {
        let childrenData = getPrefixTreeData(item[nextKey], filter, nextKey, [])
...
const findTreePath = (options, path = []) => {
  const { treeData, value, valueKey, nextKey = 'children', callback = () => {} } = options
...
const filterTreeData = (
  ...
  return data.filter((item, index) => {
    if (item[nextLevelKey] && item[nextLevelKey].length >= 1) {
      if (filterCallback(item, _index === null ? `${index}` : `${_index}-${index}`)) {
        return true
      }
